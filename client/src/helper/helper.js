...
export async function getUser({ username }){
    try {
        const { data } = await axios.get(`/user/enduser/${username}`);
        return { data };
    } catch (error) {
...
export async function updateUser(response){
    try {
        ...
        const data = await axios.put('/user/updateuser', response, { headers : { "Authorization" : `Bearer ${token}`}});
...
export async function generateOTP(username){
    try {
        ...
        if(status === 201){
            ...
            await axios.post('/user/registerMail', { username, userEmail: email, text, subject : "Password Recovery OTP"})
        }
...
export async function verifyOTP({ username, code }){
    try {
       const { data, status } = await axios.get('/user/verifyOTP', { params : { username, code }})
       return { data, status }
    } catch (error) {
        return Promise.reject(error);
    }
...
export async function getallStudent(){
    try {
        ...
        return { error : "Couldn't get all students"}
    }
}
...
export async function createClassroom(classroomData){
    try {
        const response = await axios.post('/classroom/add', classroomData);
...
export async function getallClassrooms(){
    try {
        const { data } = await axios.get(`/classroom/all`);
        return { data };
    } catch (error) {
        return { error : "Couldn't get all classrooms"}
    }
}
